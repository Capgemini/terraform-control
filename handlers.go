...
func EnvironmentIndex(w http.ResponseWriter, r *http.Request) {
	environments := RepoIndexEnvironments()
...
func EnvironmentCreate(w http.ResponseWriter, r *http.Request) {
	...
	t := RepoCreateEnvironment(environment)
	w.Header().Set("Content-Type", "application/json; charset=UTF-8")
...
func TerraformOutput(w http.ResponseWriter, r *http.Request) {
	...
	cmd := exec.Command("cat /Users/Enxebre/vagrantvm/rmg-vagrant/www/terraform-control-example/test")
	cmd.Stdout = &fw
	cmd.Stderr = &fw
	cmd.Run()
}
